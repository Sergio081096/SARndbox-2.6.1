/***********************************************************************
WaterTable2: Clase para simular el agua que fluye sobre una superficie 
utilizando una simulación de flujo de agua mejorada basada en el 
sistema Saint-Venant de ecuaciones diferenciales parciales.
...
WaterTable2::DataItem::DataItem(void)
	:currentBathymetry(0),
	 ...
	 bathymetryFramebufferObject(0),
	 derivativeFramebufferObject(0),
	 maxStepSizeFramebufferObject(0),
	 integrationFramebufferObject(0),
	 waterFramebufferObject(0),
	 bathymetryShader(0),
	 waterAdaptShader(0),
	 derivativeShader(0),
	 maxStepSizeShader(0),
	...
	/* Inicialice todas las extensiones OpenGL requeridas: */
...
WaterTable2::DataItem::~DataItem(void)
	{
	/* Eliminar todos los sombreadores, texturas y buffers asignados: */
...
void WaterTable2::calcTransformations(void)
	{
	/* Calcule la vista de modelo combinada y la matriz de proyección para renderizar imágenes de profundidad en la cuadrícula de batimetría: */
	{
	...
	/* Calcule la vista de modelo y la matriz de proyección combinadas para convertir la geometría de adición de agua en la textura del agua: */
	{
	...
	/* Convierta la matriz de adición de agua al formato OpenGL principal de la columna: */
	GLfloat* wapPtr=waterAddPmvMatrix;
	for(int j=0;j<4;++j)
		for(int i=0;i<4;++i,++wapPtr)
			*wapPtr=GLfloat(waterAddPmv.getMatrix()(i,j));
	
	/* Calcule una transformación del espacio de la cámara en el espacio de textura del agua: */
	...
	/* Convierta la transformación de la textura del agua al formato de columna principal de OpenGL: */
...
GLfloat WaterTable2::calcDerivative(WaterTable2::DataItem* dataItem,GLuint quantityTextureObject,bool calcMaxStepSize) const
	{
	/*********************************************************************
	Paso 1: Calcular derivadas espaciales parciales, flujos parciales a 
	través de los límites de las celdas y la derivada temporal.
	*********************************************************************/
	
	/* Configure el búfer de cuadros de cálculo derivado: */
	glBindFramebufferEXT(GL_FRAMEBUFFER_EXT,dataItem->derivativeFramebufferObject);
	glViewport(0,0,size[0],size[1]);
	
	/* Configurar el sombreador de cálculo de derivada temporal: */
	...
	/* Ejecutar el cálculo de la derivada temporal: */
	glBegin(GL_QUADS);
	glVertex2i(0,0);
	glVertex2i(size[0],0);
	...
	/* Desenlazar texturas innecesarias: */
	glActiveTextureARB(GL_TEXTURE1_ARB);
	glBindTexture(GL_TEXTURE_RECTANGLE_ARB,0);
	
	/*********************************************************************
	Paso 2: Reúna el tamaño de paso máximo reduciendo la textura de
	...
	*********************************************************************/
	...
	if(calcMaxStepSize)
		{
		/* Configurar el sombreador de reducción de tamaño de paso máximo: */
		glUseProgramObjectARB(dataItem->maxStepSizeShader);
		
		/* Enlazar el buffer de cuadro de cálculo de tamaño de paso máximo: */
		glBindFramebufferEXT(GL_FRAMEBUFFER_EXT,dataItem->maxStepSizeFramebufferObject);
		
		/* Reduzca la textura de tamaño de paso máximo en una secuencia de pasos de reducción media: */
		int reducedWidth=size[0];
		int reducedHeight=size[1];
		int currentMaxStepSizeTexture=0;
		while(reducedWidth>1||reducedHeight>1)
			{
			/* Configure el buffer de cuadros de simulación para la reducción máxima del tamaño de paso */
			glDrawBuffer(GL_COLOR_ATTACHMENT0_EXT+(1-currentMaxStepSizeTexture));
			
			/* Reducir la ventana gráfica por un factor de dos: */
			...
			/* Enlazar la textura de tamaño de paso máximo actual: */
			glActiveTextureARB(GL_TEXTURE0_ARB);
		...
		/* Lea el valor final escrito en el último búfer de cuadros 1x1 reducido: */
		glReadBuffer(GL_COLOR_ATTACHMENT0_EXT+currentMaxStepSizeTexture);
		glReadPixels(0,0,1,1,GL_LUMINANCE,GL_FLOAT,&stepSize);
		
		/* Limite el tamaño del paso al rango especificado por el cliente: */
...
WaterTable2::WaterTable2(GLsizei width, GLsizei height, const GLfloat sCellSize[2])
	...
	/* Inicialice el tamaño de la tabla de agua y el tamaño de la celda: */
	size[0]=width;
	size[1]=height;
	for(int i=0;i<2;++i)
		cellSize[i]=sCellSize[i];
	
	/* Calcular un dominio de simulación: */
	for(int i=0;i<2;++i)
		{
	...
	/* Calcular las transformaciones de la capa freática: */
	calcTransformations();
	...
	/* Inicialice la cantidad del depósito de agua: */
	waterDeposit=0.0f;
	}
...
WaterTable2::WaterTable2(GLsizei width, GLsizei height, const DepthImageRenderer* sDepthImageRenderer, const Point basePlaneCorners[4])
	...
	/* Calcule la transformación del espacio de la cámara al espacio del modelo de elevación vertical: */
	...
	/* Calcular el dominio del espacio modelo de elevación vertical: */
	domain=Box::empty;
	for(int i=0;i<4;++i)
	...
	/* Calcular el tamaño de celda de la cuadrícula: */
	for(int i=0;i<2;++i)
...
void WaterTable2::initContext(GLContextData& contextData) const
	...
	{
	/* Cree las texturas batimétricas centradas en vértices, reemplazando la capa más externa de células con células fantasmas: */
	...
	{
	/* Crea las texturas de estado de cantidad centrada en la celda: */
	...
	{
	/* Crear la textura derivada temporal centrada en la célula: */
	...
	{
	/* Cree las texturas de recolección de tamaño de paso máximo centradas en celdas: */
	...
	{
	/* Crea la textura del agua centrada en las células: */
	...
	/* Protege las texturas recién creadas: */
	glBindTexture(GL_TEXTURE_RECTANGLE_ARB,0);
	...
	{
	/* Crear el buffer de marco de representación batimetría: */
	...
	/* Adjunte las texturas de batimetría al búfer de representación de batimetría: */
	for(int i=0;i<2;++i)
	...
	{
	/* Crear el buffer de marco de cálculo de derivada temporal: */
	glGenFramebuffersEXT(1,&dataItem->derivativeFramebufferObject);
	glBindFramebufferEXT(GL_FRAMEBUFFER_EXT,dataItem->derivativeFramebufferObject);
	
	/* Adjunte las texturas derivadas y de tamaño de paso máximo al búfer de cuadros de cálculo de derivadas temporales: */
	...
	{
	/* Cree el búfer de cuadros de cálculo de tamaño de paso máximo: */
	glGenFramebuffersEXT(1,&dataItem->maxStepSizeFramebufferObject);
	glBindFramebufferEXT(GL_FRAMEBUFFER_EXT,dataItem->maxStepSizeFramebufferObject);
	
	/* Adjunte las texturas de tamaño de paso máximo al búfer de cuadros de cálculo de tamaño de paso máximo: */
	for(int i=0;i<2;++i)
	...
	{
	/* Crear el buffer de marco de paso de integración: */
	...
	/* Adjunte la cantidad de texturas al buffer de cuadro de pasos de integración: */
	for(int i=0;i<3;++i)
	...
	{
	/* Crear el buffer de marco de agua: */
	...
	/* Conecte la textura del agua al amortiguador de la estructura del agua: */
	...
	/* Restaura el búfer de cuadros previamente enlazado: */
	glBindFramebufferEXT(GL_FRAMEBUFFER_EXT,currentFrameBuffer);
	
	/* Cree un sombreado de vértice simple para representar quads en espacio de píxeles: */
	...
	/* Crear el sombreador de actualización batimetría: */
	{
	...
	/* Crea el shader de adaptación al agua: */
	{
	GLhandleARB vertexShader=glCompileVertexShaderFromString(vertexShaderSource);
	...
	/* Crear el sombreador de cálculo de derivada temporal: */
	{
	GLhandleARB vertexShader=glCompileVertexShaderFromString(vertexShaderSource);
	...
	/* Cree el sombreador de recopilación de tamaño de paso máximo: */
	{
	GLhandleARB vertexShader=glCompileVertexShaderFromString(vertexShaderSource);
	...
	/* Crear el sombreador de condiciones de contorno: */
	{
	GLhandleARB vertexShader=glCompileVertexShaderFromString(vertexShaderSource);
	...
	/* Cree el sombreador de pasos de integración de Euler: */
	{
	GLhandleARB vertexShader=glCompileVertexShaderFromString(vertexShaderSource);
	...
	/* Cree el sombreador de pasos de integración Runge-Kutta: */
	{
	GLhandleARB vertexShader=glCompileVertexShaderFromString(vertexShaderSource);
	...
	/* Cree el sombreador de renderizado del sumador de agua: */
	{
...
void WaterTable2::setElevationRange(Scalar newMin,Scalar newMax)
	{
	std::cout<<"13.1: SetElevationRange " << std::endl;
	/* Establecer el nuevo rango de elevación: */
	domain.min[2]=newMin;
	domain.max[2]=newMax;
	
	/* Recalcular las transformaciones de la capa freática: */
	calcTransformations();
	}
...
void WaterTable2::addRenderFunction(const AddWaterFunction* newRenderFunction)
	{
	std::cout<<"13.3: AddRenderFunction " << std::endl;
	/* Almacena la nueva función de render: */
...
void WaterTable2::removeRenderFunction(const AddWaterFunction* removeRenderFunction)
	{
	/* Encuentre la función de renderización dada en la lista y elimínela: */
...
void WaterTable2::updateBathymetry(GLContextData& contextData) const
	...
	/* Compruebe si la textura actual de la batimetría está desactualizada: */
	if(dataItem->bathymetryVersion!=depthImageRenderer->getDepthImageVersion())
		{
		/* Guardar el estado relevante de OpenGL: */
		...
		/* Enlazar el búfer de renderizado de batimetría y borrarlo: */
		glBindFramebufferEXT(GL_FRAMEBUFFER_EXT,dataItem->bathymetryFramebufferObject);
		...
		/* Hacer la superficie en la red de batimetría: */
		depthImageRenderer->renderElevation(bathymetryPmv,contextData);
		
		/* Configure el búfer de marcos de integración para actualizar las cantidades conservadas en función de los cambios de batimetría: */
		glBindFramebufferEXT(GL_FRAMEBUFFER_EXT,dataItem->integrationFramebufferObject);
		glDrawBuffer(GL_COLOR_ATTACHMENT0_EXT+(1-dataItem->currentQuantity));
		glViewport(0,0,size[0],size[1]);
		
		/* Configurar el sombreador de actualización de batimetría: */
		...
		/* Compruebe si se solicitó la red batimetría actual: */
		if(readBathymetryReply!=readBathymetryRequest)
			{
			/* Lea la red de batimetría en el búfer suministrado: */
		...
		/* Ejecutar la actualización de batimetría: */
		glBegin(GL_QUADS);
		glVertex2i(0,0);
		glVertex2i(size[0],0);
		glVertex2i(size[0],size[1]);
		...
		/* Desenlazar todos los sombreadores y texturas: */
...
void WaterTable2::updateBathymetry(const GLfloat* bathymetryGrid,GLContextData& contextData) const
	...
	/* Sube la nueva grilla de batimetría: */
	glActiveTextureARB(GL_TEXTURE1_ARB);
	glBindTexture(GL_TEXTURE_RECTANGLE_ARB,dataItem->bathymetryTextureObjects[1-dataItem->currentBathymetry]);
	glUniform1iARB(dataItem->bathymetryShaderUniformLocations[1],1);
...
void WaterTable2::setWaterLevel(const GLfloat* waterGrid,GLContextData& contextData) const
	...
	/* Configure el marco de integración para adaptar el nuevo nivel de agua a la batimetría actual: */
	glPushAttrib(GL_VIEWPORT_BIT);
	GLint currentFrameBuffer;
	glGetIntegerv(GL_FRAMEBUFFER_BINDING_EXT,&currentFrameBuffer);
	glBindFramebufferEXT(GL_FRAMEBUFFER_EXT,dataItem->integrationFramebufferObject);
	...
	/* Actualizar la cuadrícula de cantidad: */
	dataItem->currentQuantity=1-dataItem->currentQuantity;
	}
...
GLfloat WaterTable2::runSimulationStep(bool forceStepSize,GLContextData& contextData) const
	...
	/*********************************************************************
	Paso 1: Calcular la derivada temporal de las cantidades más recientes.
	*********************************************************************/
	
	GLfloat stepSize=calcDerivative(dataItem,dataItem->quantityTextureObjects[dataItem->currentQuantity],!forceStepSize);
	
	/*********************************************************************
	Paso 2: Realice el paso tentativo de integración de Euler.
	*********************************************************************/
	...
	/* Configure el buffer de cuadros de integración de pasos de Euler: */
	...
	/* Ejecute el paso de integración de Euler: */
	glBegin(GL_QUADS);
	glVertex2i(0,0);
	glVertex2i(size[0],0);
	glVertex2i(size[0],size[1]);
	...
	/*********************************************************************
	Paso 3: Calcular la derivada temporal de cantidades intermedias.
	*********************************************************************/
	
	calcDerivative(dataItem,dataItem->quantityTextureObjects[2],false);
	
	/*********************************************************************
	Paso 4: Realice el paso final de integración Runge-Kutta.
	*********************************************************************/
	
	/* Configure el buffer de cuadro de integración de pasos Runge-Kutta: */
	glBindFramebufferEXT(GL_FRAMEBUFFER_EXT,dataItem->integrationFramebufferObject);
	glDrawBuffer(GL_COLOR_ATTACHMENT0_EXT+(1-dataItem->currentQuantity));
	glViewport(0,0,size[0],size[1]);
	
	/* Configure el sombreador de pasos de integración Runge-Kutta: */
	...
	/* Ejecute el paso de integración Runge-Kutta: */
	glBegin(GL_QUADS);
	glVertex2i(0,0);
	glVertex2i(size[0],0);
	glVertex2i(size[0],size[1]);
	...
	if(dryBoundary)
		{
		/* Configure el sombreador de condiciones de contorno para imponer límites secos: */
		glUseProgramObjectARB(dataItem->boundaryShader);
		glActiveTextureARB(GL_TEXTURE0_ARB);
		glBindTexture(GL_TEXTURE_RECTANGLE_ARB,dataItem->bathymetryTextureObjects[dataItem->currentBathymetry]);
		glUniform1iARB(dataItem->boundaryShaderUniformLocations[0],0);
		
		/* Ejecute el sombreador de condiciones de contorno en la capa más externa de píxeles: */
	...
	/* Actualizar las cantidades actuales: */
	dataItem->currentQuantity=1-dataItem->currentQuantity;
	...
	if(waterDeposit!=0.0f||!renderFunctions.empty())
		...
		/*******************************************************************
		Paso 5: renderice todas las fuentes de agua y los sumideros de 
		manera aditiva en la textura del agua.
		*******************************************************************/
		
		/* Configura y limpia el buffer del marco de agua: */
		glBindFramebufferEXT(GL_FRAMEBUFFER_EXT,dataItem->waterFramebufferObject);
		glViewport(0,0,size[0],size[1]);
		...
		/* Configurar el agua añadiendo shader: */
		...
		/* Enlazar la textura del agua: */
		glActiveTextureARB(GL_TEXTURE0_ARB);
		glBindTexture(GL_TEXTURE_RECTANGLE_ARB,dataItem->waterTextureObject);
		glUniform1iARB(dataItem->waterAddShaderUniformLocations[2],0);
		
		/* Llama a todas las funciones de render: */
		...
		/****************************************************************************
		Paso 6: Actualice las cantidades conservadas basadas en la textura del agua.
		****************************************************************************/
		
		/* Configure la memoria intermedia del marco de integración para actualizar las cantidades conservadas en función de la textura del agua: */
		glBindFramebufferEXT(GL_FRAMEBUFFER_EXT,dataItem->integrationFramebufferObject);
		glDrawBuffer(GL_COLOR_ATTACHMENT0_EXT+(1-dataItem->currentQuantity));
		glViewport(0,0,size[0],size[1]);
		
		/* Configurar el sombreador de actualización de agua: */
		...
		/* Ejecutar la actualización de agua: */
		glBegin(GL_QUADS);
		glVertex2i(0,0);
		glVertex2i(size[0],0);
		glVertex2i(size[0],size[1]);
	...
	/* Devuelve el tamaño de paso del paso de Runge-Kutta: */
	return stepSize;
	}
...
void WaterTable2::bindQuantityTexture(GLContextData& contextData) const
	...
	/* Enlazar las cantidades conservadas textura: */
	glBindTexture(GL_TEXTURE_RECTANGLE_ARB,dataItem->quantityTextureObjects[dataItem->currentQuantity]);
	}
...
bool WaterTable2::requestBathymetry(GLfloat* newReadBathymetryBuffer)
	{
	/* Compruebe si la solicitud de batimetría anterior se ha cumplido: */
	if(readBathymetryReply==readBathymetryRequest)
		{
		/* Configure la nueva solicitud de batimetría: */
